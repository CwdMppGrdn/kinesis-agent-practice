...
# cloudwatch-agent、ssm-agentのインストール
dnf install -y amazon-cloudwatch-agent amazon-ssm-agent
...
aws ssm get-parameter --region ap-northeast-1 --name "/kinesis-agent/config" --output json | sudo tee /etc/aws-kinesis/agent.json
